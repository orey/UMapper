...
public class OperationProcessor implements Process
{
     ...
     * @param ops
     */
    public OperationProcessor(BufferedReader inputfilereader,
            BufferedWriter outputfilewriter,
            BufferedWriter errorfilewriter,
            BufferedWriter rejectedlineswriter,
    ...
    public int process()
    {
        ...
        try
        {
            while ((line = inputfilereader.readLine()) != null)
            {
                ...
                CSVLine input = new CSVLine(line), 
                        output = new CSVLine(targetcolnb);
                try {
                    ...
                    if (verbose) System.out.println("Operation processor: Processed line : "+
                            output.serializeCSVLine());
                    nblinetreated++;
                }
                catch (MappingException e)
                {
                    errorfilewriter.write("- One line has not been mapped : " 
                            + input.serializeCSVLine());
                    ...
                    if (verbose) System.out.println("Operation processor: Line could not " +
                            "be processed (see error file)");
                    nblineerror++;
                }
            }
